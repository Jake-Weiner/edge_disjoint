...
int main(int argc, const char** argv)
{
    ...
    std::cout << "Running " << (ap.useVol ? "Volume" : "LaPSO") << " for disjoint paths problem with "
              << ap.graph_file << " & " << ap.pairs_filename << std::endl;
    ED ed(ap.graph_file, ap.pairs_filename, ap.printing, ap.randComm, ap.djikstras_naive, ap.RREM, ap.RAEM);
    ...
    if (ap.write_outputs) {
        ...
        try {
            ...
            outfile << ap.graph_file << "," << ap.pairs_filename << "," << solver.param.nCPU << "," << solver.param.nParticles << "," << solver.param.absGap << "," << solver.param.maxIter << "," << solver.param.perturbFactor << "," << solver.param.subgradFactor
...
void write_iteration_checks(string filename, vector<T>& input_vec)
{
    ...
    if (outfile) {
        for (vector<T>::iterator it = input_vec.begin(); it != input_vec.end(); it++) {
