...
void
AnyOption::setCommandLongPrefix( const char *_prefix )
{
    if( strlen( _prefix ) > MAX_LONG_PREFIX_LENGTH ){
	strncpy(long_opt_prefix,_prefix,MAX_LONG_PREFIX_LENGTH-1);
	*( long_opt_prefix + (MAX_LONG_PREFIX_LENGTH-1) ) = '\0'; 
    } else
...
void
AnyOption::useCommandArgs( int _argc, const char **_argv )
{
	argc = _argc;
	argv = _argv;
	command_set = true;
...
void
AnyOption::processCommandArgs( int _argc, const char **_argv, int max_args )
{
	max_legal_args = max_args;
...
void
AnyOption::processCommandArgs( int _argc, const char **_argv )
{
...
char 
AnyOption::parsePOSIX(const char* arg )
{
...
int
AnyOption::parseGNU( const char *arg )
{
	int split_at = 0;
...
int 
AnyOption::matchOpt(const char *opt )
{
