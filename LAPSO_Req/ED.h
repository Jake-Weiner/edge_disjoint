...
public:
    ...
    ,graph_edges(edges),commodities(comm),num_nodes(n),nHeur(0)
    {
    ...
    int nHeur; // number of calls to heuristic    
};
...
public:
    ...
    Edge_Int_Map EIM; // note: may have both (a,b) and (b,a) which are effectively parallel edges
    vector<Edge> graph_edges;
    int solution_cost; // objective value of solution
    //Note: edges may be duplicated in the data (allowing 2 paths to connect the same i,j pair!
  ...
  */
  ...
  const int getCommSize() const {return commodities.size();}
...
private:
    ...
    std::vector<std::vector<NodeEdgePair> > node_neighbours;
